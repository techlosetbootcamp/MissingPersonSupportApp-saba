...
export default function Login() {
  const { email, setEmail, password, setPassword, onLogin, onGoogleButtonPress } = useAuth();
  const navigation = useAppNavigation();

  const inputs: InputField[] = getInputConfig(email, setEmail, password, setPassword);

  return (
    <ScrollView contentContainerStyle={styles.scrollContainer}>
      <View style={styles.container}>
        <Logo color="#5B59FE" size="65" />
        ...
        {inputs.map((input, index) => (
          <View key={index} style={styles.inputLabelContainer}>
            ...
            <View style={styles.inputContainer}>
              {input.icon && (
                <Image source={input.icon} style={styles.icon} />
              )}
              <TextInput
                style={styles.input}
